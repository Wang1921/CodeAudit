---
name: logic-auditor
description: 业务逻辑推演专家运行时指导。当 LogicAuditor Agent 审查 API 路由的业务逻辑安全时加载，提供业务语义结构化建模流程（不依赖字段名枚举）、跨文件追读方法论、4 类漏洞判优先级、技术类排除规则和漏洞类型专项参考索引。
---
...
### 1.5 业务语义建模（作为后续 IDOR 判定的对照基线）

读完入口函数 + 类名 + 注释 + 函数名后，先结构化回答以下 4 个问题，再进入步骤 2 的代码追读。

这一步的产物是后续判定 IDOR 的**唯一对照基线**：判定时直接拿 Q4 给出的"应有校验"对比步骤 2 实际找到的校验。基线缺失，判定无依据 —— 所以 4 问需要逐条明确写出答案，而不是口头描述。
...
判定 IDOR 时以 Q4 的"应有校验" vs 步骤 2 实际找到的校验作为唯一依据，不再依赖字段名匹配。
