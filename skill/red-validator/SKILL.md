...
完成攻击验证分析后，**必须在响应最后输出一个 JSON 代码块**。这是下游路由和报告生成的唯一数据来源，缺失将导致链路断裂。
...
1. **必须在响应末尾输出 ```` ```json ... ``` ```` 代码块**，包含上述 JSON 之一
2. **PoC 示例不要用 JSON 代码块包裹** —— 用普通代码块或行内格式展示 PoC，避免与最终输出 JSON 混淆
3. **不要在分析文本中输出独立的 JSON 对象**（如 `{"expression": "...", "cmd": "..."}`），这会被下游误解析为最终输出
4. 分析过程用自然语言 + 普通 Java/Python 代码块即可，最终结论用一个 ```` ```json ``` ```` 代码块收尾

### 绝对禁止
- 禁止修改 `vuln_type`
- 禁止仅输出 `{"status": "NOT_EXPLOITABLE"}` 无 defense_analysis
- 禁止以"教学/演示项目"作为 NOT_EXPLOITABLE 理由
- 禁止在分析文本中输出与最终 JSON 格式不同的裸 JSON 对象
